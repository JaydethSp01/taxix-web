...
export function DiezmosModule() {
  ...
  // PERSISTE los aportes entre recargas (libro real de diezmos/ofrendas)
  const _ld = useRef(false);
  useEffect(() => { try { const s = window.localStorage.getItem("sd_diezmos"); if (s) setAportes(JSON.parse(s)); } catch {} _ld.current = true; }, []);
